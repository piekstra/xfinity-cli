...
- **Output: text is primary.** Resource reads render `Key: value` blocks and
  ...
  and the raw `api` payload) — with one carve-out: the **utility/v1 domain
  profile** commands (`summary`, `balance`, `billing statements`) emit the
  shared profile DTOs (`utility-summary/v1`, `statement-list/v1` from
  `pk-cli-utility`) under the global `--json`; their text output is unchanged.
  Do **not** add `--json` to any other resource read. Data → stdout,
